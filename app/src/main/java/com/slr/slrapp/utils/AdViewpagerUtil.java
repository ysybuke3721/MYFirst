...
import com.slr.slrapp.adapters.ImageViewPagerAdapter;
    ...
    private void setImg(int length, String[] urls)
    {
        if (urls.length > 0) {
            ...
            for (int i = 0; i < length; i++) {
                if (i < length - 2) {
                    final int index = i;
                    Picasso.with(context) .load(urls[i]).into(mImageViews[i + 1]);
            ...
            Picasso.with(context) .load(urls[urls.length - 1]).into(mImageViews[0]);
            Picasso.with(context) .load(urls[0]).into(mImageViews[length - 1]);
        }
    }
